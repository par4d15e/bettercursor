...
├── README.md  PRD.md  SYNC_DESIGN.md  AGENTS.md
├── docs/local/              # 本地归档笔记 (gitignore, 见 docs/README.md)
```
...
> `vibe-replay-*` / `chatcrystal-*` 文章快照目录默认 **gitignore**; 需要时从上游网页或 git 历史自行恢复, 见 [docs/README.md](docs/README.md).
...
- **设计文档优先**: 涉及功能、接口、数据结构、流程或业务规则的非平凡改动,先编辑 `PRD.md` / `SYNC_DESIGN.md` 明确边界,再动手改代码。Bugfix / 局部样式 / 重命名等小改动不必走这一步。
...
- 重写前在 `docs/local/BACKGROUND.md`（本地归档, gitignore）的 "History Normalization" 小节记录 SHA 映射 (old → new) 与动机, 便于后人考古 `git blame`
