...
> ✅ **v0.2.1 + v0.2.2 + v0.2.3 + v0.2.5 已完工** (2026-07-04). v0.2.5 是**旁路 housekeeping** (跨平台打包 + i18n), 不动 sync 架构; 后续 milestone 顺延: v0.2.6 → **v0.3.0 大版本** (开本文件所有 §2-§7 的能力) → v0.3.1 (UI 层).
...
| **v0.2.6** | 跨设备 sync (Tailscale SSH 或 SSH/rsync); 这里**第一次**引入 §4 transport trait | 4-5d | 🟡 待做 |
...
总计: v0.2.x 已有 + 5 天内可完成 v0.2.4; v0.3.x 大版本 ~21 天拍板后开干.
...
v0.2-alpha ✅ ──► v0.2.1 ✅ ──► v0.2.2 ✅ ──► v0.2.3 ✅ ──► v0.2.4
                              ...
                              (Transport trait 初版, 仅 SSH/rsync 一份)
                                              │
                                              ▼
                                          v0.3.0 (unified.db + codec + conflict enum)
                                              │
                                              ▼
                                          v0.3.1 (SSH default + outbox + conflict UI + lock)
                                              │
                                ┌─────────────┴──────────────┐
                                ▼                            ▼
...
- **v0.2.4 跟 v0.3.x 的区别**: v0.2.4 是**沿用 v0.2 inline-write 路径**做跨设备, 不引入 unified.db; v0.3.0 才是 unified.db 上了的"真" multi-machine.
- **v0.3.0 是分水岭**: 用户可以选 v0.2.4 (一两个月能用) 永久, 也可以跳过 0.2.x 直接上 0.3.0 (但要承担 5 天停更).
...
| `src-tauri/src/core/transport/mod.rs` | 🆕 NEW | — | §4 trait 定义 |
| `src-tauri/src/core/transport/ssh.rs` | 🆕 NEW | — | §5 SSH/rsync default impl |
